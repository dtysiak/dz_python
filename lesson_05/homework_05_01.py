...
filtered = []
for brand, detail in car_data.items():
    if detail[1] >= search_criteria[0] and detail[2] >= search_criteria[1] and detail[4] <= search_criteria[2]:
        filtered.append((brand, detail))
def price(item):
    return item[1][4]
filtered = sorted(filtered, key=price)
for brand, detail in filtered[:5]:
    print(f"{brand}: {detail[0]}, {detail[1]}, {detail[2]}, {detail[3]}, {detail[4]}")